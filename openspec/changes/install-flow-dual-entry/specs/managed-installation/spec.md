...
`testpilot --update [REF]` SHALL run before normal Click dispatch, re-resolve the install manifest (at `REF` when provided, otherwise the latest release), reinstall the pinned core/plugin/serialwrap wheels into the managed virtualenv, and reconcile the virtualenv to exactly the manifest's component set by uninstalling any installed `testpilot.plugins` distribution not in the manifest. It SHALL capture a pre-update snapshot (e.g. `pip freeze`) before changing the environment, and on post-update `--verify-install` failure SHALL be able to restore the previous pinned set.
