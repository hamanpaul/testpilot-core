# plugin-sample-reference Specification

## Purpose
規範 testpilot-core 須提供一個最小可運行的 sample plugin,作為 `testpilot.api` SDK 的對照範例:以獨立 pip 發布單元形態、經 `testpilot.plugins` entry-point 被 host 發現、僅依賴公開 `testpilot.api` 表面,並能對其範例 case 產出明確 Pass verdict,供第三方 plugin 開發者照抄。
## Requirements
