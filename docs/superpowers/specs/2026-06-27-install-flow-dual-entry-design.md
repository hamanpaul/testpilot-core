...
- **`install-manifest.yaml`（core）**：pin core + 每個 plugin（version + **所需 api_version**）+ serialwrap；
  每個 component 標 `public/private` 與是否需 auth。CI 加 **API 相容性閘**：對每組
  `(core, plugin)` 用 `PluginLoader` 規則比對 `api_version` vs `API_VERSION`，不相容則 fail manifest PR。
...
協調 issue（追蹤順序；各 repo PR body 用 Closes #N，R-17）
 ...
 └─ Phase D 收尾
      ...
      serialwrap canonical repo 釐清（hamanpaul vs paulc-arc）後才 pin
...
**A3. brcm_fw_upgrade PR — `feature/wheel-release-readiness`**
- 前置：**先稽核**其打包（dist 名、version 來源、`testpilot.plugins` entry-point、`api_version`、韌體 blob/secret 是否誤入 wheel）。
- 內容/驗收：同 A2（依稽核結果調整）。**稽核未過前，不納入 manifest 的「install all」預設**，僅 `--plugins` 顯式可裝。

### Phase B — 先發 plugin/serialwrap wheel release

**B1.** cut `wifi_llapi`、`brcm_fw_upgrade`（若 A3 稽核過）的第一個帶 wheel asset 的 release。
`serialwrap` 的 wheel release **gated on R-2**（canonical repo 釐清，見 D2）——在 R-2 解決前，
線上路徑沿用 serialwrap source/既有安裝，離線 bundle 暫不納 serialwrap（或以已釐清來源補入）。
...
**D2. serialwrap**：釐清 canonical repo（`hamanpaul` vs `paulc-arc`），讓 install.sh 預設、README、manifest 三者一致後才 pin；若 `paulc-arc` 則該 PR 走 en_US。
...
- **R-1 brcm_fw_upgrade 未稽核**：其打包形狀（dist 名、version、entry-point、api_version、韌體 blob 是否誤入 wheel、site-packages 路徑假設）尚未驗證；Phase A3 前不納 manifest 預設。
- **R-2 serialwrap repo 身分矛盾**：本機 origin 為 `git@github.com:hamanpaul/serialwrap.git`，但 install.sh 預設與 README 寫 `paulc-arc/serialwrap`；pin 前必須釐清。
