...
def test_prompt_is_bounded_and_redacts_secret_values_without_mutating_input():
    case = {
        ...
        "steps": [{"id": "s1", "command": "password=opaque-password", "capture": []}],
    }
    policy = {"mode": "sequential", "retry": {"max_attempts": 3}, "private": "opaque-policy-secret"}
    original = repr(case)
    prompt = build_case_planning_prompt(case=case, execution_policy=policy, run_metadata={"run_id": "r1"})
...
def test_prompt_serialization_failures_are_validation_errors():
    ...
    with pytest.raises(CasePlanningValidationError):
        build_case_planning_prompt(case=BrokenMapping(), execution_policy={}, run_metadata={})
...
@pytest.mark.parametrize("raw", [
    ...
    '{"risk_summary":"password=leaked","attention_points":[],"expected_observations":[]}',
