...
def test_all_versions_are_0_3_6() -> None:
    """Release target: VERSION, pyproject.toml, and __init__.py must all be 0.3.8."""
    assert _version_file_version() == "0.3.8"
    assert _pyproject_version() == "0.3.8"
    assert _runtime_version() == "0.3.8"
...
def test_version_plugin_failures_are_isolated(monkeypatch) -> None:
    ...
    assert result.exit_code == 0
    assert result.output.startswith("TestPilot 0.3.8 (")
